...
int main(int argc, char* argv[]) {
    ...
    if (mode == "--generate-keys") {
        ...
        if (argc < 6) { 
            std::cerr << "Insufficient arguments for encryption.\n"; 
            return 1; 
        }
        std::string inF = argv[2], encFile = argv[3], encKeyFile = argv[4], pubFile = argv[5];
        auto aesKey = he.genAESKey();
        if(aesKey.empty()) { std::cerr << "AES key generation failed.\n"; return 1; }
        auto iv = he.genIV();
        if(iv.empty()) { std::cerr << "IV generation failed.\n"; return 1; }
        if (!he.aesEncFile(inF, encFile, aesKey, iv)) {
            std::cerr << "AES file encryption failed.\n";
            return 1;
        }
        auto encKey = he.encAESKey(aesKey, pubFile);
        if(encKey.empty()) { std::cerr << "AES key encryption failed.\n"; return 1; }
        std::ofstream ofs(encKeyFile, std::ios::binary);
        if (!ofs) { std::cerr << "Failed to open file for writing encrypted key.\n"; return 1; }
        ...
        if (argc < 6) { 
            std::cerr << "Insufficient arguments for decryption.\n"; 
            return 1; 
        }
        std::string encFile = argv[2], outF = argv[3], encKeyFile = argv[4], privFile = argv[5];
        std::ifstream ifs(encKeyFile, std::ios::binary);
        if (!ifs) { std::cerr << "Failed to open encrypted key file.\n"; return 1; }
        std::vector<unsigned char> encKey((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
        ifs.close();
        auto aesKey = he.decAESKey(encKey, privFile);
        if(aesKey.empty()) { std::cerr << "Failed to decrypt AES key.\n"; return 1; }
        if (!he.aesDecFile(encFile, outF, aesKey)) {
            std::cerr << "AES file decryption failed.\n";
            return 1;
        }
